				 ...
				 struct scan_control *sc)
{
	...
	if (is_active_lru(lru)) {
		if (inactive_list_is_low(mz, file))
			shrink_active_list(nr_to_scan, mz, sc, file);
		return 0;
	}

	return shrink_inactive_list(nr_to_scan, mz, sc, file);
}
...
out:
	for_each_evictable_lru(lru) {
		...
		if (sc->priority || noswap || !vmscan_swappiness(mz, sc)) {
			scan >>= sc->priority;
			if (!scan && force_scan)
...
restart:
	...
	if (inactive_anon_is_low(mz))
		shrink_active_list(SWAP_CLUSTER_MAX, mz, sc, 0);
